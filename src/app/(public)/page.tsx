...
function HeroSkeleton() {
  return (
    <div className="bg-[#0c1929] pt-16 pb-20 sm:pt-20 sm:pb-24 lg:pt-28 lg:pb-32">
...
async function SchoolProfileSection() {
  const schoolProfile = await prisma.schoolProfile.findFirst({ where: { published: true } });
...
async function PrincipalSection() {
  const principalProfile = await prisma.principalProfile.findFirst({ where: { published: true } });
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-8 max-w-3xl mx-auto">
          <div className="flex items-start gap-6">
            {principalProfile.photo && (
              <img src={principalProfile.photo} alt={principalProfile.name} width={80} height={80} loading="lazy" className="w-20 h-20 rounded-full object-cover" />
            )}
            <div>
